...
IsExtendedLocale(
    _In_ LCID Lcid)
{
    /* Check if the LCID is outside the basic locale range */
    if (((Lcid <= MAX_PRIMARY_LANGUAGE) && (Lcid != LOCALE_INVARIANT)) ||
        (Lcid == 0x0460) /* ks-Arab */ || ((Lcid & 0xFFFF) > MAX_NON_EXTENDED_LCID))
    {
        return TRUE;
    }
    ...
    _In_ BOOLEAN AllocateDestinationString)
{
    ...
    /* Check if extended locales were requested */
    if ((Flags & 0x2) == 0)
    {
        /* Check if this is an extended locale */
        if (IsExtendedLocale(Lcid))
        {
            DPRINT("RtlLcidToLocaleName: Extended LCID: 0x%lx\n", Lcid);
            return STATUS_INVALID_PARAMETER_1;
        }
    }
    ...
    _In_ ULONG Flags)
{
    ...
    if (Lcid == NULL)
    {
        DPRINT1("RtlLcidToLocaleName: Lcid is NULL\n");
        return STATUS_INVALID_PARAMETER_2;
    }
    ...
    if ((Flags & 0x2) == 0)
      {
        /* Check if the LCID is outside the basic locale range */
        if (IsExtendedLocale(FoundLcid))
        {
            DPRINT("RtlpLocaleNameToLcidInternal: Extended LCID: 0x%lx\n", FoundLcid);
            return STATUS_INVALID_PARAMETER_1;
        }
    }
